...
const { Event, Participant, User, Room, Department } = require('../../infrastructure/database/models');
const { notifyUsers } = require('../../infrastructure/external-services/services/notification.service');
const { eventToICS } = require('../../infrastructure/external-services/services/ics');
const { checkMultipleUsersConflicts } = require('../../infrastructure/external-services/services/business-trip-validator');
...
async function createEvent(req, res) {
  try {
    ...
    if (parts.length) {
      // Check for business trip conflicts for all participants
      const conflicts = await checkMultipleUsersConflicts(parts, start_time, end_time);
      const conflictedUsers = conflicts.filter(c => c.hasConflict);
      if (conflictedUsers.length > 0) {
        // Delete the event that was just created since we can't add participants
        await event.destroy();
        const conflictMessages = conflictedUsers.map(c => c.message).join('; ');
        return res.status(409).json({ message: `Không thể tạo lịch: ${conflictMessages}` });
      }
    ...
    if (deptList.length > 1) {
      const { EventDepartment } = require('../../infrastructure/database/models');
...
async function updateEvent(req, res) {
  try {
    ...
    // Restrict editing details if already approved unless admin
    if (event.status === 'approved' && !isAdmin && (title || description || start_time || end_time || roomId)) {
      return res.status(400).json({ message: 'Cannot edit approved event details' });
    }
  ...
  await event.save();
    ...
    // Notifications: status change OR detail change (pending/not approved)
    const detailChanged = (title || description || start_time || end_time || typeof roomId !== 'undefined');
    if (typeof status !== 'undefined' || (detailChanged && event.status !== 'approved')) {
      ...
      if (typeof status !== 'undefined') {
        notifyTitle = status === 'approved' ? 'Lịch được duyệt' : status === 'rejected' ? 'Lịch bị từ chối' : 'Cập nhật lịch';
        notifyBody = `${event.title} - Trạng thái: ${event.status}`;
      } else {
        notifyTitle = 'Cập nhật lịch (chưa duyệt)';
        notifyBody = `Chi tiết lịch "${event.title}" đã thay đổi`; 
      }
      await notifyUsers(ids, notifyTitle, notifyBody, { ref_type: 'event', ref_id: event.id });
    }

  const updated = await Event.findByPk(event.id, { include: [Room, { model: User, as: 'createdBy', attributes: ['id','name','email'] }, Participant] });
  try { require('../../infrastructure/external-services/services/socket').getIO().emit('dataUpdated', { resource: 'events', action: 'update', id: updated.id }); } catch (_) {}
...
async function deleteEvent(req, res) {
  try {
    ...
    if (event.status === 'approved' && req.user.role !== 'admin') {
      return res.status(400).json({ message: 'Only admin can delete approved events' });
    }
  await event.destroy();
  try { require('../../infrastructure/external-services/services/socket').getIO().emit('dataUpdated', { resource: 'events', action: 'delete', id: event.id }); } catch (_) {}
