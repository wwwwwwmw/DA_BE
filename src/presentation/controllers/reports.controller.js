const { sequelize, Event, User, Department, Participant } = require('../../infrastructure/database/models');
...
/**
 * Manager department summary: counts of tasks and events in own department.
 * GET /api/reports/departmentSummary
 */
async function departmentSummary(req, res) {
  try {
    if (!req.user || req.user.role !== 'manager') return res.status(403).json({ message: 'Forbidden' });
    const deptId = req.user.departmentId || null;
    // Tasks summary
    const taskRows = await sequelize.models.Task.findAll({
      attributes: ['status', [sequelize.fn('COUNT', sequelize.col('id')), 'count']],
      where: { departmentId: deptId },
      group: ['status'],
      raw: true,
    });
    const taskMap = { todo:0,in_progress:0,completed:0 };
    taskRows.forEach(r => { taskMap[r.status] = Number(r.count); });
    // Events summary (pending/approved/rejected/completed) for department
    const eventRows = await sequelize.models.Event.findAll({
      attributes: ['status', [sequelize.fn('COUNT', sequelize.col('id')), 'count']],
      where: { departmentId: deptId },
      group: ['status'],
      raw: true,
    });
    const eventMap = { pending:0, approved:0, rejected:0, completed:0 };
    eventRows.forEach(r => { eventMap[r.status] = Number(r.count); });
    return res.json({ tasks: taskMap, events: eventMap });
  } catch (e) { return res.status(500).json({ message: e.message }); }
}

module.exports.departmentSummary = departmentSummary;
/**
