const { Project, Task } = require('../../infrastructure/database/models');
...
async function listProjects(req, res) {
  try {
    ...
    const projects = await Project.findAll({
      where,
      include: [{ model: Task, attributes: ['id','status','weight'], include: [{ model: require('../../infrastructure/database/models').TaskAssignment, as: 'assignments', attributes: ['progress','status'] }] }],
    ...
    const mapped = projects.map(p => {
      ...
      // Fix calculation: percentage should be progressWeightedSum / totalWeightUsed * 100
      const overall = totalWeightUsed === 0 ? 0 : Math.round((progressWeightedSum / totalWeightUsed) * 100);
      const json = p.toJSON();
...
async function createProject(req, res) {
  try {
    ...
    if (createEvent && eventStart && eventEnd) {
      try {
        const { Event } = require('../../infrastructure/database/models');
        await Event.create({
          title: `[Dự án] ${name}`,
