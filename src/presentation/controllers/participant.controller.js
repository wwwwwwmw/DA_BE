const { Participant, Event, User } = require('../../infrastructure/database/models');
...
async function requestAdjustment(req, res) {
  try {
    ...
    try { require('../../infrastructure/external-services/services/socket').getIO().emit('dataUpdated', { resource: 'participants', action: 'update', id: part.id }); } catch (_) {}
    return res.json(part);
  } catch (e) { return res.status(500).json({ message: e.message }); }
}
